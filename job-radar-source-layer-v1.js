(()=>{
'use strict';
const S=window.__KA_SOURCE_STATE||{live:[],baseline:[],updatedAt:null,contentUpdatedAt:null,feedVerificationAt:null,feedSourceVerificationAt:null,baselineUpdatedAt:null,auditUpdatedAt:null,auditSummary:null,auditMode:null};
window.__KA_SOURCE_STATE=S;
const $=id=>document.getElementById(id);
const AUDIT_UI_WARN_MIN=360;
const CONTENT_UI_WARN_MIN=4320;
const JOB_VERIFY_WARN_MIN=2880;
...
function auditNoDelta(){const a=S.auditSummary||{};return a.freshnessStatus==='fresh-audit-no-live-delta'||(Object.prototype.hasOwnProperty.call(a,'freshPromoted')&&Number(a.freshPromoted)===0)||(Object.prototype.hasOwnProperty.call(a,'newActiveJobs')&&Number(a.newActiveJobs)===0)}
function renderSourceRadar(){
  ...
  const contentAge=ageMinutes(S.contentUpdatedAt||S.updatedAt),jobVerifyAge=ageMinutes(S.feedVerificationAt),sourceVerifyAge=ageMinutes(S.feedSourceVerificationAt),auditAge=ageMinutes(S.auditUpdatedAt),noDelta=auditNoDelta();
  ...
  $('sourceFresh').textContent=`LIVE obsah: ${ageLabel(contentAge)} • ponuky overené: ${ageLabel(jobVerifyAge)} • zdroje: ${ageLabel(sourceVerifyAge??auditAge)}${noDelta&&auditOperational?' • bez novej vhodnej delty':''}`;
  const health=$('sourceHealth');let warnings=[];
  if(contentAge!==null&&contentAge<-5)warnings.push('čas LIVE obsahu je v budúcnosti');
  if(jobVerifyAge!==null&&jobVerifyAge<-5)warnings.push('čas overenia ponúk je v budúcnosti');
  ...
  if(jobVerifyAge===null)warnings.push('chýba overenie LIVE ponúk');
  else if(jobVerifyAge>JOB_VERIFY_WARN_MIN)warnings.push('LIVE ponuky neboli overené viac než 48 h');
  ...
  const fh=window.JobRadarFeedHealth||{},fallback=fh.fallback,q=fh.quarantined||{};
  if(fallback&&fallback!=='none')warnings.push('feed beží cez '+fallback+' fallback');
  if((q['link-unverified']||0)>0)warnings.push(`${q['link-unverified']} promotion položiek bez link-checku`);
  if((q['low-confidence']||0)>0)warnings.push(`${q['low-confidence']} promotion položiek pod hranicou istoty`);
  health.classList.toggle('warn',warnings.length>0);health.classList.toggle('ok',warnings.length===0);
  if(warnings.length){health.textContent='⚠ Diverzita/čerstvosť: '+warnings.join(' • ')}
  else if(noDelta&&contentAge!==null&&contentAge>120){health.textContent='✓ Zdroje overené • LIVE obsah bez novej vhodnej delty'}
  else{health.textContent='✓ Zdrojová diverzita a čerstvosť v norme'}
}
...
window.fetch=async(input,init)=>{const url=typeof input==='string'?input:(input&&input.url)||'';const r=await nativeFetch(input,init);try{if(url.includes('jobs-data.json')){r.clone().json().then(d=>{S.live=d.jobs||[];S.updatedAt=d.updatedAt||null;S.contentUpdatedAt=d.contentUpdatedAt||d.updatedAt||null;S.feedVerificationAt=d.verificationUpdatedAt||null;S.feedSourceVerificationAt=d.sourceVerificationAt||null;renderSourceRadar()}).catch(()=>{})}else if(url.includes('baseline-jobs.json')){r.clone().json().then(d=>{S.baseline=d.jobs||[];S.baselineUpdatedAt=d.updatedAt||null;renderSourceRadar()}).catch(()=>{})}}catch{}return r};
