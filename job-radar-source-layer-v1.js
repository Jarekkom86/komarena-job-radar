(()=>{
'use strict';
const S=window.__KA_SOURCE_STATE||{live:[],baseline:[],updatedAt:null,baselineUpdatedAt:null,auditUpdatedAt:null};
...
function ageMinutes(ts){if(!ts)return null;const n=new Date(ts).getTime();return Number.isFinite(n)?Math.max(0,Math.round((Date.now()-n)/60000)):null}
function ageLabel(mins){if(mins===null)return'čakám';if(mins<60)return mins+' min';const h=Math.floor(mins/60);if(h<48)return h+' h';return Math.floor(h/24)+' d'}
function renderSourceRadar(){
  ...
  const feedAge=ageMinutes(S.updatedAt),auditAge=ageMinutes(S.auditUpdatedAt);
  $('sourceMix').textContent=`LIVE mix: Profesia ${liveProf}% • mimo Profesia ${100-liveProf}% • spolu ${liveN} čerstvých`;
  $('sourceTotal').textContent=`Celý radar: Profesia ${totalProf}% • mimo Profesia ${100-totalProf}% • ${allN} unikátnych`;
  $('sourceFresh').textContent=`LIVE feed: ${ageLabel(feedAge)} • audit zdrojov: ${ageLabel(auditAge)}`;
  const health=$('sourceHealth');let warnings=[];
  if(feedAge===null)warnings.push('chýba čas LIVE feedu');
  else if(feedAge>120)warnings.push('LIVE ponuky sú staršie než 2 h');
  if(auditAge===null)warnings.push('čakám na audit zdrojov');
  else if(auditAge>60)warnings.push('audit zdrojov je starší než 60 min');
  ...
  health.textContent=warnings.length?'⚠ Diverzita/čerstvosť: '+warnings.join(' • '):'✓ Zdrojová diverzita a čerstvosť v norme';
}
