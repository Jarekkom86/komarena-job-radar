...
const cleanCompany=s=>norm(s).replace(/\b(s r o|a s|spol s r o|sro|as)\b/g,' ').replace(/\s+/g,' ').trim();
const cleanTitle=s=>norm(s).replace(/\b(m z|m f d|m f n|muz zena|male female|vhodne pre absolventov)\b/g,' ').replace(/\s+/g,' ').trim();
const identity=j=>cleanCompany(j.company)+'|'+cleanTitle(j.title);
function sourceJobKey(j){
  if(j?.sourceJobId)return 'explicit:'+norm(j.sourceJobId);
  const u=canon(j?.url||'');
  let m=u.match(/profesia\.sk\/[^?#]*\/O(\d+)/i);if(m)return'profesia:'+m[1];
  m=u.match(/kariera\.zoznam\.sk\/pracovna-ponuka\/(\d+)/i);if(m)return'kariera:'+m[1];
...
function better(next,old){
  ...
  if(nh!==oh){if(nh&&nt>=ot)return true;if(oh&&ot>=nt)return false}
  const ne=!nr,oe=!or;
  if(ne!==oe){if(ne&&!oh)return true;if(oe&&!nh)return false}
...
function reduceBy(list,keyFn,collisionKey,stats){
  const map=new Map();
  list.forEach((j,i)=>{const k=keyFn(j)||'__'+i;if(map.has(k))stats[collisionKey]++;const old=map.get(k);if(better(j,old)){if(old)stats.replacements++;map.set(k,j)}});
  return [...map.values()];
}
...
function sanitizeDeltaNames(list){
  ...
  const keep=Math.max(0,14-base.length);return [...base,...dated.slice(-keep)];
}
function localDateStamp(d){return String(d.getFullYear())+String(d.getMonth()+1).padStart(2,'0')+String(d.getDate()).padStart(2,'0')}
function dynamicDeltaFallback(days=7){const files=['jobs-fresh-delta.json'];for(let i=days-1;i>=0;i--){const d=new Date();d.setHours(12,0,0,0);d.setDate(d.getDate()-i);files.push('jobs-fresh-delta-'+localDateStamp(d)+'.json')}return sanitizeDeltaNames(files)}
function staleIndex(meta){const t=Date.parse(meta?.data?.updatedAt||meta?.savedAt||'');return !t||Date.now()-t>FRESH_MS}
async function resolveDeltaFiles(){
  const idx=await getJson(DELTA_INDEX,false,validDeltaIndex);
  const indexed=sanitizeDeltaNames(idx.data?.files),fallback=dynamicDeltaFallback(7),indexStale=staleIndex(idx);
  const useFallback=!indexed.length||indexStale;
...
window.fetch=async(input,init)=>{
  ...
  try{
    ...
    const allParts=[main,extra,...deltas],merged=mergeJobsDetailed(...allParts.map(x=>x.data.jobs||[])),raw=merged.jobs,jobs=raw.filter(eligible);
    health(main,extra,deltas,deltaMeta,usedBaseline,jobs,raw,merged.stats);
    const contentUpdatedAt=latestField(allParts,'updatedAt'),verificationUpdatedAt=window.JobRadarFeedHealth.latestJobVerifiedAt,sourceVerificationAt=window.JobRadarFeedHealth.latestSourceVerificationAt;
